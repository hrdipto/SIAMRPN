...
number = []
for item in list1:
    number.append(len(os.listdir(data_dir+item+'/frames/')))
...
#        print(index)
        ...
        img = os.listdir(self.root_dir + folder + '/frames/')[index]
        img = Image.open(self.root_dir + folder + '/frames/' + img)
        if img.mode != 'RGB':
            img = img.convert('RGB')
            
        gtbox = os.listdir(self.root_dir + folder + '/anno/')[index]
        with open(self.root_dir + folder + '/anno/' + gtbox) as f:
            gtbox = f.read().split(',')
        ...
        img = os.listdir(self.root_dir + folder + '/frames/')[index + rand]
        img = Image.open(self.root_dir + folder + '/frames/' + img)
        if img.mode != 'RGB':
            img = img.convert('RGB')
        
        gtbox = os.listdir(self.root_dir + folder + '/anno/')[index + rand]
        with open(self.root_dir + folder + '/anno/' + gtbox) as f:
            gtbox = f.read().split(',')
        gtbox = [round(float(i)) for i in gtbox]
        gtbox = x1y1wh_to_xywh(gtbox)
...
#        img = F.normalize(img, [0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ...
    """Anchor Label to Anchor Detection
    """
